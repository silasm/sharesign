...
pub fn encrypt_share(cert: &pgp::Cert, share: data::Share) -> Result<data::EncryptedShare, error::SharkSignError> {
...
*/
#[cfg(test)]
pub fn decrypt_share(cert: &pgp::Cert, share: data::EncryptedShare) -> Result<data::Share, error::SharkSignError> {
    Ok(data::Share {
...
pub fn generate(approvers: &[pgp::Cert], shares_needed: u8, config: &data::KeyConfig) -> Result<data::GeneratedKey, error::SharkSignError> {
    ...
    for (cert, shark) in approvers.iter().zip(dealer.take(approvers.len())) {
        ...
        let signature = pgp::sign(&key, &share_bytes)?;
        shares.push(encrypt_share(cert, data::Share {
            data: share_bytes,
...
mod tests {
    ...
    fn test_generate_shares_rsa_2048() {
        ...
        let shares_plaintext: Vec<data::Share> = generated.shares.into_iter().zip(td.approvers_priv().iter()).map(
            move |(share, key)| {
                decrypt_share(&key, share).unwrap()
            }
        ).collect();
